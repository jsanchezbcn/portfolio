...
- [ ] T030 [US3] Implement `ExecutionEngine.submit(order: Order) -> Order` — call `POST /iserver/account/{acctId}/orders`; build `conidex` multi-leg payload for combos; poll order status until FILLED/REJECTED/CANCELLED or timeout; return updated `Order` with fill details; mark status "unknown" on connection loss
- [ ] T031 [US3] Add 2-step confirmation modal to `dashboard/components/order_builder.py` — Step 1: preview panel (legs, quantities, order type, simulated margin); Step 2: "Confirm & Submit" button; Step 3: disable both buttons after transmission
...
## Phase 6: User Story 4 — Trade Journal with Context Capture (Priority: P3)
